...
class Model {
	...
	void initGame() throws Exception {
		...
		this.lowestCost = 1 / this.getTravelSpeed(0.0f, 0.0f);
		for(float x = 0.0f; x < Model.XMAX; x += 10) { //find lowest cost
			for(float y = 0.0f; y < Model.YMAX; y += 10) {
				float newCost = 1 / this.getTravelSpeed(x, y);
				if(this.lowestCost > newCost) {
					this.lowestCost = newCost; 
				}
			}
		}
	}
